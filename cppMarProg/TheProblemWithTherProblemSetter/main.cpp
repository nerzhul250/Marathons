...
int main()
{
    ...
    while(cin >> nk){
        cin >> np;
        if(np==0 && nk==0)break;
        PushRelabel<int> mfGraph(np+nk+2);
        int sum=0;
        for(int i=0;i<nk;i++){
            int problemCat;
            cin >> problemCat;
            sum+=problemCat;
            mfGraph.AddEdge(np+i+1,np+nk+1,problemCat);
        }
        for(int j=0;j<np;j++){
            int num;
            cin >> num;
            mfGraph.AddEdge(0,j+1,1);
            for(int i=0;i<num;i++){
                int cat;
                cin >> cat;
                 mfGraph.AddEdge(j+1,np+cat,INF);
            }
        }
        int mf=mfGraph.GetMaxFlow(0,np+nk+1);
        if(mf==sum){
            cout << "1\n";
            for(int i=0;i<nk;i++){
                int theCat=i+1+np;
                bool first=true;
                for(int j=0;j<mfGraph.adj[theCat].size();j++){
                    if(mfGraph.adj[theCat][j].to!=np+nk+1 && mfGraph.adj[theCat][j].flow==-1){
                        if(first){
                            cout << mfGraph.adj[theCat][j].to;
                            first=false;
                        }else{
                            cout << " " <<mfGraph.adj[theCat][j].to;
                        }
